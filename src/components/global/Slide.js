...
export const Slide = React.memo(function (StackedCarouselSlideProps) {
  ...
  useEffect(() => {
    if (isCenterSlide) {
      const observedVideoElement = ref && ref.current;
      console.log(observedVideoElement, "play!");
      observedVideoElement.play();
    } else {
      const observedVideoElement = ref && ref.current;
      observedVideoElement.pause();
  ...
  return (
    <div
      className="card-card"
      style={{ border: `solid 2px ${colors[dataIndex]}` }}
      draggable={false}
    >
      <Container
        onClick={() => {
      ...
      >
        ...
        <TopProfile>
          <UrlImageLoader
            round={100}
            imgUrl={`https://avatars0.githubusercontent.com/u/${Math.ceil(
              Math.random() * 10000
            )}?v=4`}
            w={45}
          />
